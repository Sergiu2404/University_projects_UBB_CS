...
#define EXIT "3"
		...
		options.push_back(ADMIN_ROLE);
		options.push_back(USER_ROLE);
		options.push_back(EXIT);

		try {
			this->validator.validate(option, options);
		}
		catch (InputException inputError) {
			std::cout << "[EXCEPTION]" << inputError.print_message();
		}
		options.clear();
...
#define EXIT_ADMIN "5"
				...
				options.push_back(ADD_ADMIN);
				options.push_back(REMOVE_ADMIN);
				options.push_back(UPDATE_ADMIN);
				options.push_back(DISPLAY_ADMIN);
				options.push_back(EXIT_ADMIN);

				try {
					this->validator.validate(option_admin, options);
				}
				catch (InputException inputError) {
					std::cout << "[Exception] " << inputError.print_message();
				}
...
#define EXIT_USER "4"
				...
				options.push_back(SEE_USER);
				options.push_back(REMOVE_USER);
				options.push_back(DISPLAY_USER);
				options.push_back(EXIT_USER);

				try {
					this->validator.validate(option_user, options);
				}
				catch(InputException inputError){
					std::cout << "[Exception] " << inputError.print_message();
				}
...
#define SEE_NO "no"
	...
	if (genre == "")
	{
		...
		while (currentSize < this->service.get_adminRepo().size())
		{
			...
			int position = -1;
			std::vector<Movie> movies = this->service.get_userRepo();
			for (int i = 0; i < movies.size(); i++)
				if (movies[i].get_title() == movie.get_title() && movies[i].get_genre() == movie.get_genre())
					position = i;
