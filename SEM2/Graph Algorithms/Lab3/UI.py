...
class UI:
    ...
    def backwards_dijkstra(self):

        # 2.
        # Write a program that, given a graph with positive costs and two vertices, finds a lowest cost walk between the given vertices,
        # using a "backwards" Dijkstra algorithm (Dijkstra algorithm that searches backwards, from the ending vertex).

        vertex1=int(input("vertex number 1: "))
        vertex2=int(input("vertex number 2: "))

        if(self.__graphs[self._current].backwards_dijkstra(vertex1,vertex2)==None):
            print(f"no way from source: {vertex1} to target: {vertex2}")
        else:
            path,costs=self.__graphs[self._current].backwards_dijkstra(vertex1,vertex2)
            print(f"Path by edges: {path}")
    ...
    def start(self):
        ...
        while True:
            #try:
                ...
                elif option.lower()=="backwards_dijkstra":
                    self.backwards_dijkstra()
                else:
                    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
