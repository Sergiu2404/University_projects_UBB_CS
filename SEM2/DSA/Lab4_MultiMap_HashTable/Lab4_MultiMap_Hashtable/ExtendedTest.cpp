...
void testIterator() { 
	...
	try {
		im.getCurrent();
		assert(im.getCurrent()==NULL_TELEM);
	}
	catch (exception& ex) {
		assert(true);
	}
	/*try {
		im.next();
		cout << im.getCurrent().first<<" "<<im.getCurrent().second;
		assert(false);
	}
	catch (exception& ex) {
		assert(true);
	...
	for (int i = 0; i < 100; i++) {
		...
		if (i == 0){
			...
			assert(elem.first == NULL_TVALUE && elem.second == NULL_TVALUE);
			assert(elem2.first == NULL_TVALUE && elem2.second == NULL_TVALUE);
		}
		im2.next();
	}
	assert(im2.valid() == false);
	...
	int counter = 0;
	while (im5.valid()&&counter<480) {
		TElem e = im5.getCurrent();
		elements.push_back(e);
		im5.next();
		counter++;
	}
	cout << elements.size()<<" "<<m4.size();
