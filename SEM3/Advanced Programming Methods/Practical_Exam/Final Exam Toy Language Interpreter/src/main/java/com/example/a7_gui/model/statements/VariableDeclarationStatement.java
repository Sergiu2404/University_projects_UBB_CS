...
public class VariableDeclarationStatement implements IStatement {
    ...
    @Override
    public ProgramState executeStatement(ProgramState state) throws StatementExecutionException {
        MyISymbolTable<String, Value> symTable = state.getSymbolTable();
        if (symTable.isDefined(name)) {
            throw new StatementExecutionException("VarDecl: Variable " + name + " already exists in the symTable.");
        }
