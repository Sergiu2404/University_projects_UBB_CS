...
public class Main {
    ...
    private static void grammarMenu() {
        ...
        do {
            ...
            System.out.println("16. Write Derivation steps corresponding to g1.txt and input seq.txt to out1.txt file");
            System.out.println("17. Write Derivation steps corresponding to g2.txt and input p1PIF.txt to out2.txt file");
            System.out.println("0. Exit");
            System.out.println("======================================");
            ...
            try {
                ...
                switch (option) {
                    ...
                    case 16:
                        if (parserOutput != null) {
                            // Read input sequence from seq.txt
                            try (Scanner fileScanner = new Scanner(new File("input/seq.txt"))) {
                                String sequence = fileScanner.nextLine().trim();
                                System.out.println("Sequence loaded from seq.txt: " + sequence);

                                // Parse the sequence
                                boolean accepted = parserOutput.parseSequence(sequence);

                                // Write derivation steps or error message to out1.txt
                                try (PrintWriter writer = new PrintWriter(new FileWriter("output/out1.txt"))) {
                                    if (accepted) {
                                        writer.println("Sequence accepted.");
                                        writer.println("Derivation Steps:");
                                        List<String> derivationSteps = parserOutput.getDerivationSteps();
                                        for (int i = 0; i < derivationSteps.size(); i++) {
                                            writer.println("Step " + (i + 1) + ": " + derivationSteps.get(i));
                                        }

                                        // Add the parsing tree after the derivation steps
                                        writer.println("\nParsing Tree (Father-Sibling Table):");
                                        writer.println("Node\t\tParent\t\tSibling");
                                        writer.println("--------------------------------------------");

                                        // Write the father-sibling table into the output file
                                        parserOutput.printParsingTreeWithParentSibling(parserOutput.getRoot(), null, new ArrayList<>(), writer);

                                    } else {
                                        writer.println("Sequence not accepted.");
                                    }
                                    System.out.println("Output written to output/out1.txt");
                                } catch (IOException e) {
                                    System.err.println("Error writing to output/out1.txt: " + e.getMessage());
                                }
                            } catch (FileNotFoundException e) {
                                System.err.println("Error: input/seq.txt not found.");
                            }
                        } else {
                            System.out.println("Please generate parsing table first (option 11).");
                        }
                    ...
                    case 17:
                        if (parserOutput != null) {
                            // Read input sequence from p1PIF.txt
                            try (Scanner fileScanner = new Scanner(new File("output/p1PIF.txt"))) {
                                StringBuilder sequenceBuilder = new StringBuilder();
                                int lineNumber = 1;
                                Map<Integer, String> errorDetails = new HashMap<>();

                                // Build the sequence from p1PIF.txt and track potential errors
                                while (fileScanner.hasNextLine()) {
                                    String line = fileScanner.nextLine().trim();
                                    // Assuming each line has format "token - (code, pos)"
                                    String[] parts = line.split(" - ");
                                    if (parts.length < 2 || !parts[1].matches("\\(.*\\)")) {
                                        errorDetails.put(lineNumber, "Invalid token format: " + line);
                                    } else {
                                        sequenceBuilder.append(parts[0]).append(" "); // Add token to sequence
                                    }
                                    lineNumber++;
                                }

                                String sequence = sequenceBuilder.toString().trim();
                                System.out.println("Sequence loaded from p1PIF.txt: " + sequence);

                                // Parse the sequence
                                boolean accepted = parserOutput.parseSequence(sequence);

                                // Write derivation steps or error messages to out2.txt
                                try (PrintWriter writer = new PrintWriter(new FileWriter("output/out2.txt"))) {
                                    if (!errorDetails.isEmpty()) {
                                        writer.println("Errors in input file:");
                                        for (Map.Entry<Integer, String> error : errorDetails.entrySet()) {
                                            writer.println("Line " + error.getKey() + ": " + error.getValue());
                                        }
                                        writer.println(); // Add spacing before parsing output
                                    }
                                    ...
                                    if (accepted) {
                                        ...
                                        writer.println("Sequence not accepted.");
                                        writer.println("Error during parsing: invalid token or syntax mismatch.");
                                    }
                                    System.out.println("Output written to output/out2.txt");
                                } catch (IOException e) {
                                    System.err.println("Error writing to output/out2.txt: " + e.getMessage());
                                }
                            } catch (FileNotFoundException e) {
                                System.err.println("Error: output/p1PIF.txt not found.");
                            }
                        } else {
                            System.out.println("Please generate parsing table first (option 11).");
                        }
