...
class _SymptomPage extends State<SymptomPage> {
  ...
  bool checkSaved() {
    final entries = recordsPerDate.where((element) => element == widget.date);
  ...
  getData() async {
    ...
    if (online == true) {
      ...
      if (checkSaved() == false) {
        recordsPerDate = await DatabaseContext.instance.getByDate(date);
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
  ...
  syncData() async {
    ...
    if (checkSaved()) {
      DatabaseContext.instance.deleteAllEntriesByDate(date);
      // sync data and add category to list
      for (var transaction in recordsPerDate) {
        DatabaseContext.instance.addEntry(transaction);
      }
      dateSaved.add(date);
    }
  }
  ...
  void addEntry(Entry entry) async {
    ...
    if (online) {
      try {
        ...
        DatabaseContext.instance.addEntry(newEntry);
      } on Exception catch (_) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
  ...
  void deleteEntryBack(Entry entry) async {
    ...
    if (online) {
      ...
      setState(() {
        ...
        DatabaseContext.instance.deleteEntry(entry.id!);
      });
    }
    setState(() => isLoading = false);
  }
    ...
    return Scaffold(
      ...
      body: isLoading
          ...
          itemBuilder: (context, index) {
            return ListTile(
              title: Text(recordsPerDate[index].id.toString()),
